...
contract DecentralizedGovernance is ReentrancyGuard {
    ...
    uint256 public quorumPercentage = 5;//设置参与总票数的最小百分比是5%才能有效
    uint256 public proposalDepositAmount = 10;
    //设置创建提案的时候必须锁定多少个治理token，如果提案获胜并执行可取回，阻止低质量或过多提案
    ...
    function createProposal(string calldata _description, address[] calldata _targets,
    bytes[] calldata _calldatas) external returns (uint256) {
    //calldata 数据只读，从交易的原始数据包拿，不复制到内存
    //省Gas费，声明变量immutalbe不可修改，只支持外部调用用于external函数
